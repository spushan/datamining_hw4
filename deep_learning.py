...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            acc=0
            for p, y in zip(class_predicted, y_test):
                if p==y:
                    acc += 1
            acc=acc/len(y_test)
            if acc > highestAccuracy:
                highestAccuracy = acc
                model.save('highest_model')
...
h_model = keras.models.load_model('highest_model')
img_file = './model_arch.png'
#tf.keras.utils.plot_model(h_model, to_file=img_file, show_shapes=True, show_layer_names=True)
